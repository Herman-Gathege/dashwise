...
@customers_bp.route('/top-customers', methods=['GET'])
@jwt_required()
def get_top_customers():
    ...
    results = (
        db.session.query(
            ...
            func.sum(Appointment.fee).label('total_paid')
        )
        .filter_by(user_id=user_id)
        .filter(Appointment.status == 'Complete')
        .group_by(Appointment.client_name)
        .order_by(func.sum(Appointment.fee).desc())
        .limit(5)
        .all()
    )
